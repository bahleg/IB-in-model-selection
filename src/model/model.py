...
class Setup(Function):
    ...
    def _change_weights(self):
        ...
        for i in range(len(self._scalars)):
            # maybe should add low and high as parameters
            self._scalars = np.random.uniform(
                low=-10, high=10, size=self._nscalars)

        for i in range(len(self._vectors)):
            # maybe should add low and high as parameters
            self._vectors[i] = np.random.uniform(
                low=-10, high=10, size=self._nfeatures)
...
class Learn(Function):
    ...
    def _randomize_component_function(self, pool: Pool):
        ...
        self._args[change_idx] = dict(
            arg_indices=np.random.choice(
                np.arange(pool.nscalars if arg_type ==
                          SCALAR else pool.nvectors),
                size=OPS_ARGS[func_idx]["n_args"]
            ),
            arg_type=arg_type,
            return_idx=np.random.choice(
                np.arange(pool.nscalars if return_type ==
                          SCALAR else pool.nvectors),
                size=1
            ),
            return_type=return_type
    ...
    def _change_arguments(self, pool: Pool):
        ...
        self._args[change_idx]["arg_indices"] = np.random.choice(
            np.arange(
                pool.nscalars if self._args[change_idx]["arg_type"] == SCALAR else pool.nvectors),
            size=OPS_ARGS[self._functions[change_idx]]["n_args"]
        )
        self._args[change_idx]["return_idx"] = np.random.choice(
            np.arange(
                pool.nscalars if self._args[change_idx]["return_type"] == SCALAR else pool.nvectors),
            size=1
        )
    ...
    def __repr__(self) -> str:
        ...
        for func_idx, func_arg in zip(self._functions, self._args):
            ...
            if OPS_ARGS[func_idx]["n_args"] == 1:
                ...
                if func_arg["arg_type"] == SCALAR:
                    output += f"(s{func_arg['arg_indices']
                                   [0]}, s{func_arg['arg_indices'][1]})"
                elif func_arg["arg_type"] == VECTOR:
                    output += f"(v{func_arg['arg_indices']
                                   [0]}, v{func_arg['arg_indices'][1]})"
...
class Model:
    ...
    def _validate(
        ...
        for row in val_data:
            ...
            # object is always put in address v0
        ...
        for train, valid in zip(train_datasets, valid_datasets):
            ...
            if return_predictions:
                preds = (self._validate(
                    pool, valid, normalize, loss, return_predictions))
                return preds
            else:

                val_accuracy += self._validate(pool, valid,
                                               normalize, loss, return_predictions)
